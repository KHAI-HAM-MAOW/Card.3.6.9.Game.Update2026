...
// ---------- LAO VOICE PLAYBACK ----------
// Per-rank voice files (user-recorded .wav). Stored in audio/ folder.
const VOICE_FILES = {
  '2':  'audio/voice_2.wav',
  '3':  'audio/voice_3.wav',
  '4':  'audio/voice_4.wav',
...
// Preloaded Audio elements (one per rank) — created lazily on first need
const voiceCache = {};
// Currently playing voice (so we can stop it when overlay closes)
let currentVoice = null;

function preloadVoices() {
  // Create Audio objects for all ranks; browser will request files on play.
  // We use 'auto' preload to hint the browser to fetch them when idle.
  for (const rank of Object.keys(VOICE_FILES)) {
    if (!voiceCache[rank]) {
      const a = new Audio();
      a.src = VOICE_FILES[rank];
      a.preload = 'auto';
      a.volume = 0.95;
      // suppress errors if file missing — graceful degrade
      a.addEventListener('error', () => {
        console.warn('Voice file missing or not loadable:', VOICE_FILES[rank]);
      });
      voiceCache[rank] = a;
    }
  }
...
function playVoice(rank, delay = 0) {
  if (!soundEnabled) return;
  const audio = voiceCache[rank];
  if (!audio) return;
  // stop any previous voice first
  stopVoice();
  // play after optional delay
  setTimeout(() => {
    if (!soundEnabled) return; // re-check in case user toggled during delay
    try {
      audio.currentTime = 0;
      const p = audio.play();
      if (p && typeof p.catch === 'function') {
        p.catch(() => { /* autoplay blocked or file missing — silent fail */ });
      }
      currentVoice = audio;
    } catch (e) {
      // silent
...
function getOrbitRadii() {
  ...
  // Perfect horizontal circle (lying flat on the ground), viewed from the front.
  // The circle has a single radius R; depth is rendered via perspective projection
  // (front cards lower & larger, back cards higher & smaller).
  const R = Math.min(w * 0.36, 240, h * 0.32);
  return { R };
}
...
function placeCard(card, slotIndex, slotCount, time) {
  ...
  // Card position on a HORIZONTAL circle (lying flat on the ground in 3D).
  // World coords: x = R*cos(angle) [left-right], z = R*sin(angle) [forward-back]
  // The camera is positioned above and slightly in front, looking down toward the center.
  const tilt = 0.45; // 0 = top-down view, 1 = pure side view. ~0.45 = nice 3D angle.

  const worldX = Math.cos(angle) * R;
  const worldZ = Math.sin(angle) * R; // +Z = closer to viewer (front), -Z = farther (back)

  // Screen projection: front of circle goes DOWN on screen (+Y in screen coords),
  // back goes UP (-Y).
  const screenX = worldX;
  const screenY = worldZ * tilt;

  // Depth (-1 back .. +1 front) for perspective effects
  const depth = worldZ / R;

  // Perspective scale: front cards larger, back smaller
  const scale = 0.55 + (depth + 1) * 0.30; // 0.55 (back) .. 1.15 (front)

  // z-index: front above back
  card.el.style.zIndex = String(Math.round((depth + 1) * 100));

  // Opacity fade for far-away cards
  const opacity = 0.55 + (depth + 1) * 0.225; // 0.55 (back) .. 1.0 (front)
  card.el.style.opacity = opacity.toFixed(2);

  // Gentle bobbing & wobble for life
  const bob = Math.sin(time / 700 + slotIndex * 1.3) * 4;
  const rot = Math.sin(time / 900 + slotIndex * 0.7) * 3;

  card.el.style.transform =
    `translate3d(${screenX.toFixed(2)}px, ${(screenY + bob).toFixed(2)}px, 0) scale(${scale.toFixed(3)}) rotate(${rot.toFixed(2)}deg)`;
  card.angle = angle;
}
...
function onCardTap(card) {
  ...
  requestAnimationFrame(() => {
    ...
    setTimeout(() => {
      ...
      // confetti burst
      spawnConfetti();
      // play Lao voice for this rank, slightly after the flip starts revealing the face
      playVoice(card.rank, 350);
